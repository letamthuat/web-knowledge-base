...
export function AudioFinishDialog({ blob, durationMs, onClose, onCancel }: Props) {
  const router = useRouter();
  const [fileName, setFileName] = useState(`Ghi âm ${new Date().toLocaleDateString("vi-VN")}`);
  ...
  async function handleUpload() {
    ...
    try {
      ...
      toast.success("Đã lưu vào thư viện", {
        action: {
          label: "Mở",
          onClick: () => router.push(`/reader/${doc as Id<"documents">}`),
        },
      });
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="bg-background border rounded-xl shadow-xl p-6 w-full max-w-md flex flex-col gap-4">
        <h2 className="text-base font-semibold">Lưu bản ghi âm</h2>
