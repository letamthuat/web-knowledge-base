...
import { ScreenFinishDialog } from "./ScreenFinishDialog";
import { ScreenRecorderResult } from "@/hooks/useScreenRecorder";
...
function PillContent({
  ...
  recorder: ScreenRecorderResult;
  onStop: () => void;
  size?: "sm" | "md";
}) {
  const { state, micActive, hasMic, sysAudioActive, hasSysAudio, durationMs, pause, resume, toggleMic, toggleSysAudio } = recorder;
  ...
  return (
    <>
      ...
      {hasMic && (
        <button onClick={toggleMic} title={micActive ? "Tắt mic" : "Bật mic"} className={`${btnCls} text-white`}>
          {micActive ? (
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className={iconCls}>
      ...
      {hasSysAudio && (
        <button onClick={toggleSysAudio} title={sysAudioActive ? "Tắt âm thanh màn hình" : "Bật âm thanh màn hình"} className={`${btnCls} text-white`}>
          {sysAudioActive ? (
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className={iconCls}>
      ...
      {/* Stop */}
      <button onClick={onStop} title="Dừng quay" className={btnCls}>
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className={iconCls}>
...
function useFinishResult(recorder: ScreenRecorderResult) {
  const [finishResult, setFinishResult] = useState<{ blob: Blob; durationMs: number } | null>(null);

  // Handle pending finish triggered by user stopping share from browser
  const ext = recorder as ScreenRecorderResult & {
    _pendingFinish?: { blob: Blob; durationMs: number };
    _clearPendingFinish?: () => void;
  };

  useEffect(() => {
    if (ext._pendingFinish && !finishResult) {
      setFinishResult(ext._pendingFinish);
      ext._clearPendingFinish?.();
    }
  }, [ext._pendingFinish, ext._clearPendingFinish, finishResult]);

  return { finishResult, setFinishResult };
}

/** Inline variant — dùng trong TabBar (desktop) */
export function ScreenRecordingPill() {
  const { screenRecorder } = useRecording();
  const { state, cancel } = screenRecorder;
  const { finishResult, setFinishResult } = useFinishResult(screenRecorder);

  async function handleStop() {
    const result = await screenRecorder.finish();
      ...
      {isActive && (
        <div
          className={`hidden md:flex items-center gap-1.5 rounded-full px-3 py-1 text-sm font-medium select-none transition-colors shrink-0 ${
            state === "paused" ? "bg-muted text-muted-foreground" : "bg-violet-600 text-white"
          }`}
        >
          <PillContent recorder={screenRecorder} onStop={handleStop} size="sm" />
        </div>
      )}
...
/** Floating variant — dùng trên mobile (fixed bottom-right, offset để không đè audio pill) */
export function ScreenRecordingPillFloating() {
  const { screenRecorder } = useRecording();
  const { state, cancel } = screenRecorder;
  const { finishResult, setFinishResult } = useFinishResult(screenRecorder);
  ...
  return (
    <>
      {isActive && (
        ...
        >
          <PillContent recorder={screenRecorder} onStop={handleStop} size="md" />
        </div>
      )}
