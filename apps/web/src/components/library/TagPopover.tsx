...
interface TagPopoverProps {
  docId: string;
  /** Fix A: parent (grid) truyền sẵn tags của doc → không tự mở subscription/card.
   *  Không truyền thì tự fetch (tương thích chỗ dùng lẻ). */
  docTags?: TagRow[];
}

export function TagPopover({ docId, docTags: docTagsProp }: TagPopoverProps) {
  const [open, setOpen] = useState(false);
  // Chỉ fetch khi parent KHÔNG truyền sẵn (đối số undefined → hook skip)
  const fallbackDocTags = useTagsForDoc(docTagsProp !== undefined ? undefined : docId);
  const docTags = docTagsProp ?? fallbackDocTags;
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent className="w-60 p-2" align="start">
        {/* Nội dung (và hook useTagsList bên trong) chỉ mount khi popover mở */}
        {open && <TagPopoverInner docId={docId} docTags={docTags} />}
      </PopoverContent>
    </Popover>
  );
}

function TagPopoverInner({ docId, docTags }: { docId: string; docTags: TagRow[] | undefined }) {
